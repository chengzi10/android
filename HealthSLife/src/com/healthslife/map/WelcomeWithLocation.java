...
public class WelcomeWithLocation extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (isNetworkAvailable(WelcomeWithLocation.this)) {
			// 网络连接正常
	...
	public void decideJump(Context context) {
		...
		String sql = "select * from " + SportInfoDAO.TABLENAME_UserInfo
				+ " where 1=1";
		Cursor c = dao.query(sql, null);// 查询
		// int info_id=-1;//id默认值，-1表示不存在!!
		while (c.moveToNext()) {
			/*
			 * private String user_name;//用户名---“英文字母”和“下划线”组成 private int
			 * user_sex;//性别 private int user_weight;//体重 private int
			 * user_high;//身高 private int user_aimstep;//目标步行数目 private int
			 * user_islogin;//用户登录状态
			 */
			...
			SensorData
					.setAim_stepNum(c.getInt(c.getColumnIndex("user_aimstep")));
			SensorData
					.setLogin(c.getInt(c.getColumnIndex("user_islogin")) == 1 ? true
							: false);

			System.out.println("@@@@@@@@@@在引导页查询info表结果：@"
					+ SensorData.getUsername() + "@" + SensorData.getGender()
					+ "@" + SensorData.getWeight() + "@"
					+ SensorData.getHeight() + "@"
					+ SensorData.getAim_stepNum() + "@" + SensorData.isLogin());// @@@@@@@@@@测试!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		}
