...
import com.healthslife.music.entity.Album;
import com.healthslife.music.entity.Artist;
...
public class SongDao {
	...
	public long add(Song song) {
		...
		values.put(DBData.SONG_LEVEL,song.getLevel());
	...
	public List<String[]> searchByAll() {
		...
		while (cr.moveToNext()) {
			...
			s[4] = String.valueOf(cr.getInt(cr.getColumnIndex(DBData.SONG_ISLIKE)));
			list.add(s);
		}
		cr.close();
		db.close();
	...
	private List<Song> addList(Cursor cr) {
		...
		while (cr.moveToNext()) {
			...
			song.setLevel(cr.getInt(cr
					.getColumnIndex(DBData.SONG_LEVEL)));
			list.add(song);
		}
		return list;
	}
	 ...
	 * 更新音乐的等级
	 * */
	public void updateByLevel(int id, int level) {
		SQLiteDatabase db = dbHpler.getWritableDatabase();
		db.execSQL("UPDATE " + DBData.SONG_TABLENAME + " SET "
				+ DBData.SONG_LEVEL + "=" + level + " WHERE " + DBData.SONG_ID
				+ "=" + id);
		db.close();
	}
	 ...
	 * 根据音乐等级查询歌曲信息
	 * */
	public Song searchByLevel(int level) {
		Song song = null;
		SQLiteDatabase db = dbHpler.getReadableDatabase();
		Cursor cr = db.rawQuery("SELECT  A." + DBData.SONG_DISPLAYNAME + ",B."
				+ DBData.ARTIST_NAME + " AS Bname" + ",C." + DBData.ALBUM_NAME
	 ...
	 * 根据id查询歌曲level
	 * */
	public int searchLevelById(int id) {
		int level = 0;
		SQLiteDatabase db = dbHpler.getReadableDatabase();
		Cursor cr = db.rawQuery("SELECT " + DBData.SONG_LEVEL 
				+ " FROM " + DBData.SONG_TABLENAME
				+ " WHERE " + DBData.SONG_ID,null);
	 ...
	 * 根据等级查询
	 * */
	public List<Song> searchLevel(int level) {
		return commonSearch(" WHERE A." + DBData.SONG_LEVEL + "=? ",
				new String[] { String.valueOf(level) });
	}
