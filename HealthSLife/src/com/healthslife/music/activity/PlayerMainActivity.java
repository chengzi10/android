...
import com.healthslife.music.custom.FlingGalleryView.OnCustomTouchListener;
...
import com.healthslife.music.util.ImageUtil;
...
public class PlayerMainActivity extends BaseActivity {
	...
	private static final int[] MODE_DRAWABLE_ID=new int[]{
		...
		R.drawable.player_btn_player_mode_intelligent
	};
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		registerReceiver(mediaPlayerBroadcastReceiver, new IntentFilter(MediaPlayerManager.BROADCASTRECEIVER_ACTION));
		//播放器管理
	...
	private class MediaPlayerBroadcastReceiver extends BroadcastReceiver{
		...
		public void onReceive(Context context, Intent intent) {
			...
			if(flag==MediaPlayerManager.FLAG_CHANGED){
				...
				int currentPosition=intent.getIntExtra("currentPosition", 0);		//获取当前位置
				int duration=intent.getIntExtra("duration", 0);						//获取播放时长
				int playerMode=intent.getIntExtra("playerMode", 0);					//获取播放模式
				int playerState=intent.getIntExtra("playerState", 0);				//获取播放状态
				...
				//播放模式背景设置
				ibtn_player_control_mode.setBackgroundResource(MODE_DRAWABLE_ID[playerMode]);
	...
	/**
	 * 播放器布局中按钮（音量、上一首、下一首、播放/暂停、播放模式）的点击事件绑定监听器
	 * */
	private OnClickListener listener = new OnClickListener() {
		public void onClick(View v) {
			...
			case R.id.ibtn_player_control_menu:
				new XfDialog.Builder(PlayerMainActivity.this)
							.setTitle(tv_player_song_info.getText())
							.setMessage("抱歉，暂时没有开发")
							.create()
							.show();
				break;
			case R.id.ibtn_player_control_mode:
				...
				if (player_mode == MediaPlayerManager.MODE_INTELLIGENT) {	//智能播放
					player_mode = MediaPlayerManager.MODE_CIRCLELIST;	//顺序播放
				} else {
					player_mode++;
				}
